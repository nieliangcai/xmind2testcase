...
def parse_testsuite(suite_dict):
    testsuite = TestSuite()
    testsuite.name = suite_dict.get('title')
...
def get_priority(case_dict):
    ...
    if isinstance(case_dict.get('makers'), list):
        for marker in case_dict.get('makers'):
...
def gen_testcase_title(topics):
    ...
    titles = [topic.get('title') for topic in topics]
...
def gen_testcase_summary(topics):
    comments = [topic.get('comment') if topic.get('comment') else "" for topic in topics]
...
def parse_a_test_step(step_dict):
    ...
    # 去掉数字和顿号，如：“1、”
    test_step.actions = re.sub('^\d、', '', test_step.actions)
    ...
    if expected_topics:
        ...
        for expected_topic in expected_topics:
            title = re.sub('^\d、', ' ', str(expected_topic['title'])).strip()

            if expected_topic.get('topics', []):
                # 有子步骤，有子预期（1.1.子步骤...）
                expected = re.sub('^\d、', ' ', str(expected_topic['topics'][0]['title'])).strip()

                test_step.actions += f"\n{sub_number}.{title}"
                test_step.expectedresults += f"\n{sub_number}.{expected}"
            elif len(expected_topics) > 1:
                # 有子步骤，没有子预期（1.1.子步骤...）
                test_step.actions += f"\n{sub_number}.{title}"
            else:
                # 没有子步骤（1.步骤...）
    ...
    logging.debug('finds a test step: %s', test_step.to_dict())
    return test_step
